...
class Filter1    : public ElementComponent, public TextButton::Listener, Slider::Listener, public ChangeListener, public Value::Listener
{
public:
    Filter1()
    {
    ...
    void changeListenerCallback(ChangeBroadcaster* source) override
    {
        ...
        if (source == &egFilter && egFilter.getSegmentCount() >= 6)
        {
            ...
            sliderRL2.setValue(fromPctL(egFilter.getReleaseLevel()),  dontSendNotification);
            isUpdating = false;
        }
    }
    void sliderValueChanged (Slider *slider) override
    {
        ...
        {
            isUpdating = true;
            // Levels: SY99 signed -64..+63 → 0-100% display (center=50%)
            auto toPctL = [](double val) { return (int)((val + 64.0) * 100.0 / 128.0); };
            // Rates:  SY99 unsigned 0..63 → 0-100% display
    ...
    void setElementNumber ( int element, UndoManager& um) override
    {
        ...
        // toPctL: signed level -64..+63 → 0-100%   (center=50%, -64=0%, +63≈99%)
        // toPctR: rate 0..63 → INVERTED 0-100%   (rate=63 fast → 1.6% narrow, rate=0 slow → 100% wide)
        auto toPctL = [](double val) { return (int)((val + 64.0) * 100.0 / 128.0); };
        auto toPctR = [](double val) { return (int)((64.0 - val) * 100.0 / 64.0); };
        ...
        sysexdata[6] = 0x0e;
        sysexdata2[6] = 0x0f;
        // Segment 5: start=RL1, rate=RR2 (release rate 2), end=RL2
    ...
    void paint (Graphics& g) override
    {
        ...
        DBG("Filter1 bounds: " + String(getWidth()) + "x" + String(getHeight())
    ...
    void resized() override
    {
        ...
        // Set egFilter bounds explicitly in pixels, then force its internal layout
