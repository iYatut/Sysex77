...
// initialisation et ouverture des propriétées
        initProperties();
        loadParams();
        comboModel.setSelectedId (static_cast<int> (DeviceModel::getInstance().getDevice()),
    ...
    void 	comboBoxChanged (ComboBox *comboBoxThatHasChanged) override
    {
        ...
        {
            auto id = comboModel.getSelectedId();
            if (id >= 1 && id <= 3)
                DeviceModel::getInstance().setDevice (static_cast<YamahaDevice> (id));
        }
    ...
    void buttonClicked (Button* button) override
    {
        ...
        if (button == &btColBack)       selectorName = "colBack";
        else if (button == &btColAlt)   selectorName = "colAlt";
        else if (button == &btColLab)   selectorName = "colLab";
        else if (button == &btColSel)   selectorName = "colSel";

        auto colourSelector = std::make_unique<ColourSelector>();
        colourSelector->setName (selectorName);
