...
public:
    CommonFilter()
    {
        ...
        // SwitchFilter — 3-state LP/BP/HP selector (addRadio calls are kept
        // for source-compatibility but are no-ops on SwitchFilter)
        ...
        btFilter2.setLookAndFeel(&buttonFilterLF);
        ...
        sliderFq1.setLookAndFeel(&filtreControlLF);
        ...
        sliderFq2.setLookAndFeel(&filtreControlLF);
        ...
        sliderLfoSens.setLookAndFeel(&filtreControlLF);
        ...
        sliderVelocity.setLookAndFeel(&filtreControlLF);
        ...
        sliderResonnance.setLookAndFeel(&filtreControlLF);
    ...
    ~CommonFilter()
    {
        // Clear externally-set LookAndFeels before members are destroyed
        btFilter2.setLookAndFeel(nullptr);
        sliderFq1.setLookAndFeel(nullptr);
        sliderFq2.setLookAndFeel(nullptr);
        sliderLfoSens.setLookAndFeel(nullptr);
        sliderVelocity.setLookAndFeel(nullptr);
        sliderResonnance.setLookAndFeel(nullptr);
    }
    ...
    void paint (Graphics& g) override
    {
        ...
        // Reserved area for future filter frequency response curve (top 74%)
        auto curveArea = getLocalBounds().removeFromTop((int)(getHeight() * 0.74f));
        g.setColour(Colour(0xff222222));
        g.fillRect(curveArea);
        g.setColour(Colour(0x30808080));
        g.drawRect(curveArea, 1);

        // Draw a simple mid-line as placeholder for the frequency response
        g.setColour(Colour(0x40ff8c00));
        float midY = curveArea.getY() + curveArea.getHeight() * 0.5f;
        g.drawLine(curveArea.getX() + 4.0f, midY,
                   curveArea.getRight() - 4.0f, midY, 1.5f);
    }
...
private:
    // LookAndFeel objects must be declared BEFORE the components that use them
    // so that they are destroyed AFTER those components (reverse construction order).
    FiltreControlLookAndFeel filtreControlLF;
    ButtonFilterLookAndFeel  buttonFilterLF;
    ...
    SwitchFilter radioFilter1Mode;       // replaces MidiRadio — LP/BP/HP toggle
